...
if (isset($_POST["widget"])
   ...
   if (isset($_POST["locations_id"])) {
    ...
    && ($_POST["widget"] == "PluginMydashboardInfotel32" || $_POST["widget"] == "PluginMydashboardInfotel33")) {
   if (isset($_POST["priority_id"])) {
      $options['reset']                     = 'reset';
      $options['criteria'][0]['field']      = 12; // status
      $options['criteria'][0]['searchtype'] = 'equals';
    ...
    && ($_POST["widget"] == "PluginMydashboardInfotel34" || $_POST["widget"] == "PluginMydashboardInfotel35")) {
   if (isset($_POST["priority_id"])) {
      $options['reset']                     = 'reset';

      $options['criteria'][0]['field']      = 3; // priority
      $options['criteria'][0]['searchtype'] = 'equals';
      $options['criteria'][0]['value']      = $_POST["priority_id"];
      $options['criteria'][0]['link']       = 'AND';

      if ($_POST["type"] > 0) {
         $options['criteria'][1]['field']      = 14; // type
         $options['criteria'][1]['searchtype'] = 'equals';
         $options['criteria'][1]['value']      = $_POST["type"];
         $options['criteria'][1]['link']       = 'AND';
      }

      $options['criteria'][2]['field']      = 80; // entities
      $options['criteria'][2]['searchtype'] = 'equals';
      if (isset($_POST["sons"]) && $_POST["sons"] > 0) {
         $options['criteria'][2]['searchtype'] = 'under';
      }
      $options['criteria'][2]['value'] = $_POST["entities_id"];
      $options['criteria'][2]['link']  = 'AND';
      ...
      $options['criteria'][3]['link']       = 'AND NOT';

      $options['criteria'][4]['field']      = 12; // status
      $options['criteria'][4]['searchtype'] = 'equals';
      $options['criteria'][4]['value']      = Ticket::INCOMING;
      $options['criteria'][4]['link']       = 'AND NOT';

      $options['criteria'][5]['field']      = 12; // status
      $options['criteria'][5]['searchtype'] = 'equals';
      $options['criteria'][5]['value']      = Ticket::CLOSED;
      $options['criteria'][5]['link']       = 'AND NOT';

      if (!empty($_POST["groups_id"])) {
         $options['criteria'][6]['field'] = 8; // technician group
         $options['criteria'][6]['searchtype'] = 'equals';
         $options['criteria'][6]['value'] = $_POST["groups_id"];
         $options['criteria'][6]['link'] = 'AND';
      }
