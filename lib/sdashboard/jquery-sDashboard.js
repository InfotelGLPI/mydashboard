...
( function(factory) {"use strict";
		...
		$.widget("mn.sDashboard", {
			...
			_bindEvents : function() {
				...
				this.element.on("click", ".sDashboardWidgetHeader div.sDashboard-icon.sDashboard-circle-plus-icon", function(e) {
					...
					$(e.currentTarget).toggleClass("sDashboard-circle-minus-icon");
                                        var maximizeString = (typeof self.options.dashboardLanguage.maximize !== 'undefined')? self.options.dashboardLanguage.maximize:"Maximize";
                                        var minimizeString = (typeof self.options.dashboardLanguage.minimize !== 'undefined')? self.options.dashboardLanguage.minimize:"Minimize";
					//change the tooltip on the maximize/minimize icon buttons
					if ($(e.currentTarget).attr("title") === maximizeString) {
						$(".sDashboard-overlay").show();
						$(e.currentTarget).attr("title", minimizeString);
						self._trigger("widgetMaximized", null, {
							"widgetDefinition" : widgetDefinition
						});
					} else {
						$(".sDashboard-overlay").hide();
						$(e.currentTarget).attr("title", maximizeString);
				...
				this.element.on("click", ".sDashboardWidgetHeader div.sDashboard-icon.sDashboard-circle-remove-icon ", function(e) {
					...
					widget.hide("fold", {}, 300, function() {
						...
						self._trigger("stateChanged", null,  {
							triggerAction: 'widgetDeleted',
							affectedWidgetId: this.id
						}
					);
					});
				});
			...
			_constructWidget : function(widgetDefinition) {
				...
				var maximizeString = (typeof this.options.dashboardLanguage.maximize !== 'undefined')? this.options.dashboardLanguage.maximize:"Maximize";
				var closeString = (typeof this.options.dashboardLanguage.close !== 'undefined')? this.options.dashboardLanguage.close:"Close";
                                var refreshString = (typeof this.options.dashboardLanguage.refresh !== 'undefined')? this.options.dashboardLanguage.refresh:"Refresh";
				var maximizeButton = $('<div title="'+maximizeString+'" class="sDashboard-icon sDashboard-circle-plus-icon "></span>');

				var deleteButton = $('<div title="'+closeString+'" class="sDashboard-icon sDashboard-circle-remove-icon"></div>');
				...
				if (widgetDefinition.hasOwnProperty("enableRefresh") && widgetDefinition.enableRefresh) {
					var refreshButton = $('<div title="'+refreshString+'" class="sDashboard-icon sDashboard-refresh-icon "></div>');
					//add refresh button
			...
			_renderTable : function(widgetDefinition){
				...
				if(widgetDefinition.widgetType === 'table'){
					...
					//Language handling
					//If you want Datatable in your own language
					if(typeof this.options.dashboardLanguage !== 'undefined')
					{
						$.extend(tableDef,{"oLanguage":this.options.dashboardLanguage});
					}
