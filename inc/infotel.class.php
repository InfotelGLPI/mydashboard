      ...
      switch($widgetId) {
         ...
         case $this->getType()."4":
            ...
            case $this->getType()."6":
               ...
               $query = "SELECT DATE_FORMAT(`glpi_tickets`.`date`, '%Y-%m') as month, DATE_FORMAT(`glpi_tickets`.`date`, '%b %Y') as monthname, DATE_FORMAT(`glpi_tickets`.`date`, '%Y%m') as monthnum, count(MONTH(`glpi_tickets`.`date`))
                        FROM `glpi_tickets`
                        WHERE `glpi_tickets`.`is_deleted` = 0 " . $entities . " AND MONTH(`glpi_tickets`.`date`)='" . date("m") . "' AND(YEAR(`glpi_tickets`.`date`) = '" . date("Y") . "') GROUP BY DATE_FORMAT(`glpi_tickets`.`date`, '%Y-%m')";
               ...
               while ($data = $DB->fetch_array($results)) {
                  ...
                  $query_1 = "SELECT COUNT(*) as count FROM `glpi_tickets`
                     ...
                     AND (((`glpi_tickets`.`date` <= '$year-$month-$nbdays 23:59:59') AND `status` NOT IN (" . CommonITILObject::SOLVED . "," . CommonITILObject::CLOSED . ")) OR ((`glpi_tickets`.`date` <= '$year-$month-$nbdays 23:59:59') AND (`glpi_tickets`.`solvedate` > ADDDATE('$year-$month-$nbdays 00:00:00' , INTERVAL 1 DAY))))";
