   ...
   function getWidgetsForItem() {
      return [
         _n('Alert', 'Alerts', 2, 'mydashboard') => [
            ...
            $this->getType() . "10" => __('Incident'). " : " .  __("Users answer tickets", "mydashboard") . "&nbsp;<i class='fa fa-table'></i>",
            $this->getType() . "11" => __('Request'). " : " .  __("Users answer tickets", "mydashboard") . "&nbsp;<i class='fa fa-table'></i>",
         ]
      ];
   }
   ...
   function getWidgetContentForItem($widgetId, $opt = []) {
      ...
      switch ($widgetId) {
         ...
         case $this->getType() . "10":
            ...
            $link_ticket = Toolbox::getItemTypeFormURL("Ticket");
            $groups_criteria = PluginMydashboardHelper::getGroup($this->preferences['prefered_group'],$opt);

            $entities = " AND `glpi_tickets`.`entities_id` IN  (" . implode(",", $_SESSION['glpiactiveentities']) . ") ";
            $type = " AND `glpi_tickets`.`type` = ".Ticket::INCIDENT_TYPE. " ";
            $grp_LeftJoin = "";
            $groupe = "";
            if (isset($groups_criteria) && ($groups_criteria != 0)) {
               $grp_LeftJoin =  " LEFT JOIN `glpi_groups_tickets` 
                                 ON (`glpi_groups_tickets`.`tickets_id` = `glpi_tickets`.`id`
                                     AND `glpi_groups_tickets`.`type` = '" . CommonITILActor::ASSIGN . "')";
               $groupe =  " AND `glpi_groups_tickets`.`groups_id` = " . $groups_criteria;
            }
            $query    = "SELECT  `glpi_tickets`.`id` as tickets_id,
                                          `glpi_tickets`.`status` as status,
                                          `glpi_tickets`.`date_mod` as date_mod
                                 ...
                                 WHERE `glpi_tickets`.`is_deleted` = '0'
                                 AND `glpi_tickets`.`status` != '" . CommonITILObject::CLOSED . "'
                                 AND `glpi_tickets`.`date_mod` != `glpi_tickets`.`date` $entities $type $groupe";
            ...
            $widget->setWidgetTitle("<span style='color:orange'><i class='fa fa-warning fa-1x'></i>&nbsp;" .
                __('Incident'). " : " .  sprintf(__("Users answer to %d tickets", "mydashboard"), count($datas)) . "</span>");
            $widget->setWidgetComment(__("Display tickets where last modification is a user action", "mydashboard"));
         ...
         case $this->getType() . "11":
            ...
            $entities = " AND `glpi_tickets`.`entities_id` IN  (" . implode(",", $_SESSION['glpiactiveentities']) . ") ";
            $type = " AND `glpi_tickets`.`type` = ".Ticket::DEMAND_TYPE. " ";
            $grp_LeftJoin = "";
            $groupe = "";
            if (isset($groups_criteria) && ($groups_criteria != 0)) {
               $grp_LeftJoin =  " LEFT JOIN `glpi_groups_tickets` 
            ...
            $widget->setWidgetTitle("<span style='color:orange'><i class='fa fa-warning fa-1x'></i>&nbsp;" .
                __('Request'). " : " .  sprintf(__("Users answer to %d tickets", "mydashboard"), count($datas)) . "</span>");
            $widget->setWidgetComment(__("Display tickets where last modification is a user action", "mydashboard"));
