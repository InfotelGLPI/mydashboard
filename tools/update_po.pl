	...
	if ($_ ne '..' && $_ ne '.'){
            ...
            if(!(-l "$dir/$_")){
                     if (index($_,".po",0)==length($_)-3) {
                        ...
                        `wget --user=$user --password=$password --output-document=locales/$_ http://www.transifex.net/api/2/project/GLPI_mydashobard/resource/glpi/translation/$lang/?file=$_`;
                     }
            }
