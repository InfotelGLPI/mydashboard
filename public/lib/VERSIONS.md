...
Three entries cannot be pinned there and are tracked here only:

- `diacritics.js` has no upstream version at all (single unversioned file);
- `fileSaver.min.js` is a legacy 1.3.x build carrying no version marker, so the exact
  patch level is unknown;
...
| `echarts/echarts.js` | Apache ECharts | 6.1.0 | Apache-2.0 | https://github.com/apache/echarts |
| `echarts/theme/*.js` | ECharts themes | shipped with ECharts 6.1.0 | Apache-2.0 | https://github.com/apache/echarts |
| `fileSaver.min.js` | FileSaver.js | 1.3.x (legacy `saveAs` build, no version marker) | MIT | https://github.com/eligrey/FileSaver.js |
